...
df["TPV_mensal_t2"] = df.groupby("id")["TPV_mensal"].shift(-1)
...
df_prediction_september = df_prediction.groupby("id").head(2).groupby("id").tail(1).drop("TPV_mensal_t2", axis="columns")


# # Separating training and test dataset
df = df[df["mes_referencia"] < "2020-08-30"]

df_val = df.groupby("id").tail(2).groupby("id").head(1)
df_val = df_val[pd.notnull(df_val["TPV_mensal_t2"])].set_index("mes_referencia")

df_train = df.drop(df.groupby("id").tail(1).index, axis=0).set_index("mes_referencia")
df_train = df_train[pd.notnull(df_train["TPV_mensal_t2"])]
...
x_colnames = list(df.drop("TPV_mensal_t2", axis="columns").columns.values)
...
if TUNING:
    ...
    def objective(trial, cv_fold_func=np.average):
        ...
        for train_idx, test_idx in GroupTimeSeriesSplit().split(df_train_tuning, groups=df_train_tuning["id"]):
            train_x, test_x = df_train_tuning.drop("TPV_mensal_t2", axis="columns").values[train_idx], df_train_tuning.drop("TPV_mensal_t2", axis="columns").values[test_idx]
            train_y, test_y = df_train_tuning["TPV_mensal_t2"][train_idx], df_train_tuning["TPV_mensal_t2"][test_idx]
    ...
    # # Saving best params
    filename = "best_params_september"
    ...
    infile = open("best_params_september", "rb")
...
if TRAINING:
    ...
    for train_idx, test_idx in GroupTimeSeriesSplit().split(df_train, groups=df_train["id"]):
        train_x, test_x = df_train.drop("TPV_mensal_t2", axis="columns").values[train_idx], df_train.drop("TPV_mensal_t2", axis="columns").values[test_idx]
        train_y, test_y = df_train["TPV_mensal_t2"][train_idx], df_train["TPV_mensal_t2"][test_idx]
    ...
    df_val_september_y = df_val["TPV_mensal_t2"]
    df_val_september_x = df_val.drop(["TPV_mensal_t2"], axis="columns")
    y_pred_lgb_test = lgbm_model.predict(df_val_september_x, num_iteration=lgbm_model.best_iteration)

    # # Model performance on test data
    MAE_lgb_test = mean_absolute_error(y_pred_lgb_test, df_val_september_y)
    ...
    # Saving Results
    imp.to_csv("Model_Performance/importances_september.csv")

    filename = "Model_Performance/MAE_september"
    outfile = open(filename, "wb")
    ...
    if PREDICTION:
        ...
        y_pred_lgb_final.to_csv("Predictions/prediction_september.csv")
