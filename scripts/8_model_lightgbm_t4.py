...
# # Shifting TPV_mensal three rows for 4 lag prediction (November)
df["TPV_t4"] = df.groupby("id")["TPV_mensal"].shift(-3)
...
df_prediction_november = df_prediction.groupby("id").head(4).groupby("id").tail(1).drop("TPV_t4", axis="columns")
df_prediction_november["ind"] = df_prediction_november["id"]
df_prediction_november = df_prediction_november.set_index("ind")
...
df_val = df_val[pd.notnull(df_val["TPV_t4"])].set_index("mes_referencia")
df_val = df_val.reset_index()
df_val = df_val.sort_values(["id", "mes_referencia"])
df_val = df_val.set_index("mes_referencia")

df_train = df.drop(df.groupby("id").tail(8).index, axis=0).set_index("mes_referencia")
df_train = df_train[pd.notnull(df_train["TPV_t4"])]
df_train = df_train.head(TRAINING_SIZE)

df = df.set_index("mes_referencia")
df = df[pd.notnull(df["TPV_t4"])]
...
x_colnames = list(df.drop("TPV_t4", axis="columns").columns.values)
...
if TUNING:
    ...
    def objective(trial, cv_fold_func=np.average):
        ...
        for train_idx, test_idx in GroupTimeSeriesSplit().split(df_train_tuning, groups=df_train_tuning["id"]):
            train_x, test_x = df_train_tuning.drop("TPV_t4", axis="columns").values[train_idx], df_train_tuning.drop("TPV_t4", axis="columns").values[test_idx]
            train_y, test_y = df_train_tuning["TPV_t4"][train_idx], df_train_tuning["TPV_t4"][test_idx]
    ...
    # # Saving best params
    filename = f"{data_dir}/data/model/parameters/best_params_t4.pickle"
    ...
    infile = open(f"{data_dir}/data/model/parameters/best_params_t4.pickle", "rb")
...
if TRAINING:
    ...
    for train_idx, test_idx in GroupTimeSeriesSplit().split(df_train, groups=df_train["id"]):
        train_x, test_x = df_train.drop("TPV_t4", axis="columns").values[train_idx], df_train.drop("TPV_t4", axis="columns").values[test_idx]
        train_y, test_y = df_train["TPV_t4"][train_idx], df_train["TPV_t4"][test_idx]
    ...
    df_val_t4_y = df_val["TPV_t4"]
    df_val_t4_x = df_val.drop(["TPV_t4"], axis="columns")
    y_pred_lgb_test = lgbm_model.predict(df_val_t4_x, num_iteration=lgbm_model.best_iteration)

    # # Model performance on test data
    MAE_lgb_test = mean_absolute_error(y_pred_lgb_test, df_val_t4_y)
    ...
    # Saving Results
    imp.to_csv(f"{data_dir}/data/model/performance/importances_t4.csv")

    filename = f"{data_dir}/data/model/performance/MAE_t4.pickle"
    outfile = open(filename, "wb")
    pickle.dump(MAE_lgb_test, outfile)
    outfile.close()
    
    # Joining test set and predictions
    df_y_pred = pd.DataFrame(y_pred_lgb_test, columns=["TPV_pred_t4"])
    df_val_t4_x = df_val_t4_x.reset_index()
    df_val_t4_y = pd.DataFrame(df_val_t4_y)
    df_val_t4_y = df_val_t4_y.reset_index().drop("mes_referencia", axis="columns")
    df_pred_t4 = pd.concat([df_val_t4_x, df_val_t4_y, df_y_pred], axis="columns").set_index("mes_referencia")
    df_pred_t4 = df_pred_t4[["id", "TPV_t4", "TPV_pred_t4"]]

    # Saving dataframe
    df_pred_t4.to_csv(f"{data_dir}/data/performance/test_pred_t4.csv")
...
if FINAL_PREDICTION:
    ...
    for train_idx, test_idx in GroupTimeSeriesSplit().split(df, groups=df["id"]):
        train_x, test_x = df.drop("TPV_t4", axis="columns").values[train_idx], df.drop("TPV_t4", axis="columns").values[test_idx]
        train_y, test_y = df["TPV_t4"][train_idx], df["TPV_t4"][test_idx]
    print("Beginning training...")
    train_data = lgb.Dataset(train_x, label=train_y, categorical_feature=categorical_vars, feature_name=x_colnames)
    test_data = lgb.Dataset(test_x, label=test_y, categorical_feature=categorical_vars, feature_name=x_colnames)
    ...
    # Creating validation sets for each month
    y_pred_lgb_final = lgbm_model.predict(df_prediction_november, num_iteration=lgbm_model.best_iteration)

    # # Exporting final results
    y_pred_lgb_final = pd.DataFrame(y_pred_lgb_final, columns=["TPV Novembro"])
    y_pred_lgb_final.to_csv(f"{data_dir}/data/predictions/prediction_november.csv")
