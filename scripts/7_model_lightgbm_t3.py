...
# # Shifting TPV_mensal two rows for 3 lag prediction (October)
df["TPV_t3"] = df.groupby("id")["TPV_mensal"].shift(-2)
...
df_prediction_october = df_prediction.groupby("id").head(3).groupby("id").tail(1).drop("TPV_t3", axis="columns")
df_prediction_october["ind"] = df_prediction_october["id"]
df_prediction_october = df_prediction_october.set_index("ind")
...
df_val = df_val[pd.notnull(df_val["TPV_t3"])].set_index("mes_referencia")
df_val = df_val.reset_index()
df_val = df_val.sort_values(["id", "mes_referencia"])
df_val = df_val.set_index("mes_referencia")

df_train = df.drop(df.groupby("id").tail(7).index, axis=0).set_index("mes_referencia")
df_train = df_train[pd.notnull(df_train["TPV_t3"])]
df_train = df_train.head(TRAINING_SIZE)

df = df.set_index("mes_referencia")
df = df[pd.notnull(df["TPV_t3"])]
...
x_colnames = list(df.drop("TPV_t3", axis="columns").columns.values)
...
if TUNING:
    ...
    def objective(trial, cv_fold_func=np.average):
        ...
        for train_idx, test_idx in GroupTimeSeriesSplit().split(df_train_tuning, groups=df_train_tuning["id"]):
            train_x, test_x = df_train_tuning.drop("TPV_t3", axis="columns").values[train_idx], df_train_tuning.drop("TPV_t3", axis="columns").values[test_idx]
            train_y, test_y = df_train_tuning["TPV_t3"][train_idx], df_train_tuning["TPV_t3"][test_idx]
    ...
    # # Saving best params
    filename = f"{data_dir}/data/model/parameters/best_params_t3.pickle"
    ...
    # # Reading Boruta vars
    infile = open(f"{data_dir}/data/model/parameters/best_params_t3.pickle", "rb")
...
if TRAINING:
    ...
    for train_idx, test_idx in GroupTimeSeriesSplit().split(df_train, groups=df_train["id"]):
        train_x, test_x = df_train.drop("TPV_t3", axis="columns").values[train_idx], df_train.drop("TPV_t3", axis="columns").values[test_idx]
        train_y, test_y = df_train["TPV_t3"][train_idx], df_train["TPV_t3"][test_idx]
    ...
    df_val_t3_y = df_val["TPV_t3"]
    df_val_t3_x = df_val.drop(["TPV_t3"], axis="columns")
    y_pred_lgb_test = lgbm_model.predict(df_val_t3_x, num_iteration=lgbm_model.best_iteration)

    # # Model performance on test data
    MAE_lgb_test = mean_absolute_error(y_pred_lgb_test, df_val_t3_y)
    ...
    # Saving Results
    imp.to_csv(f"{data_dir}/data/model/performance/importances_t3.csv")

    filename = f"{data_dir}/data/model/performance/MAE_t3.pickle"
    outfile = open(filename, "wb")
    pickle.dump(MAE_lgb_test, outfile)
    outfile.close()
    
    # Joining test set and predictions
    df_y_pred = pd.DataFrame(y_pred_lgb_test, columns=["TPV_pred_t3"])
    df_val_t3_x = df_val_t3_x.reset_index()
    df_val_t3_y = pd.DataFrame(df_val_t3_y)
    df_val_t3_y = df_val_t3_y.reset_index().drop("mes_referencia", axis="columns")
    df_pred_t3 = pd.concat([df_val_t3_x, df_val_t3_y, df_y_pred], axis="columns").set_index("mes_referencia")
    df_pred_t3 = df_pred_t3[["id", "TPV_t3", "TPV_pred_t3"]]

    # Saving dataframe
    df_pred_t3.to_csv(f"{data_dir}/data/performance/test_pred_t3.csv")
...
if FINAL_PREDICTION:
    ...
    for train_idx, test_idx in GroupTimeSeriesSplit().split(df, groups=df["id"]):
        train_x, test_x = df.drop("TPV_t3", axis="columns").values[train_idx], df.drop("TPV_t3", axis="columns").values[test_idx]
        train_y, test_y = df["TPV_t3"][train_idx], df["TPV_t3"][test_idx]
    print("Beginning training...")
    train_data = lgb.Dataset(train_x, label=train_y, categorical_feature=categorical_vars, feature_name=x_colnames)
    test_data = lgb.Dataset(test_x, label=test_y, categorical_feature=categorical_vars, feature_name=x_colnames)
    ...
    # Creating validation sets for each month
    y_pred_lgb_final = lgbm_model.predict(df_prediction_october, num_iteration=lgbm_model.best_iteration)

    # # Exporting final results
    y_pred_lgb_final = pd.DataFrame(y_pred_lgb_final, columns=["TPV Outubro"])
    y_pred_lgb_final.to_csv(f"{data_dir}/data/predictions/prediction_october.csv")
